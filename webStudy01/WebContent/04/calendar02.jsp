...
<%
	...
	String tmZone = request.getParameter("timezone");
	TimeZone timeZone = today.getTimeZone();
	if (tmZone != null) {
		today.getTimeZone().setID(tmZone);
	}
...
<body>
	<h4>
		현재 서버의 시각 :
		<%=String.format(locale, "%tc", today)%></h4>
	<%-- <%=String.format("%tc", cal) %> --%>
	<h4>
		<!-- 	(calendar.jsp)? -->
		<a href="#" class="moveA" data-year="<%=beforeYear%>"
			data-month="<%=beforeMonth%>">이전달</a>
		<!-- 시간연도 4글자, (t시간)b(로케일에따른'월'데이터), cal, cal  -->
		<%=String.format(locale, "%1$tY, %1$tB", cal)%>
		<a href="#" class="moveA" data-year="<%=nextYear%>"
			data-month="<%=nextMonth%>">다음달</a>
	</h4>
		...
		<input type="number" name="year" placeholder="<%=year%>"
			...
			<%
				...
				for (String tmpTime : timezones) {
					//String selected = tmpTime.equals(today.getTimeZone().toZoneId().toString())?"selected":"";
					String selected = (tmpTime.equals(tmZone)) ? "selected" : "";

					out.println(String.format(optionPtrn, selected, tmpTime, tmpTime));
				}
			%>
		</select>
	...
	<script>
		function nowDisplay() {
			...
			document.frm.time.value = nowTime
			//1초간격으로 함수호출

			window.setTimeout("nowDisplay()", 1000);
	...
	<script type="text/javascript">
		...
		// :input모든 input 태그 사용
		var calForm = $("#calendarForm").on("change", ":input", function() {
